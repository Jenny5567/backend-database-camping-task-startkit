...
set role = 'COACH'
where role =  'USER' 
and  email in('lee2000@hexschooltest.io','muscle@hexschooltest.io','starplatinum@hexschooltest.io');
...
-- 2-1. 新增：在`CREDIT_PACKAGE` 資料表新增三筆資料，資料需求如下：
    ...
    -- 為什麼CREDIT_PACKAGE 已經有價錢跟堂數 CREDIT_PURCHASE 也有價錢跟堂數？
    -- Ans: CREDIT_PURCHASE 可能價錢跟堂數經過折價跟加碼 所以跟CREDIT_PACKAGE不同

insert into "CREDIT_PACKAGE" (name , price , credit_amount)
values
	('7 堂組合包方案',  1400 ,  7 ), -- 注意價錢不要逗號 不然會出錯
	('14 堂組合包方案', 2520 , 14 ),
	('21 堂組合包方案', 4800 , 21 );
...
-- 2-2. 新增：在 `CREDIT_PURCHASE` 資料表，新增三筆資料：（請使用 name 欄位做子查詢）
    ...
    values
    ((select id from "USER" where email = 'wXlTq@hexschooltest.io'), -- 使用者id, email會設定unique
    (select id from "CREDIT_PACKAGE" where name = '14 堂組合包方案'), -- 組合包id
    (select credit_amount from "CREDIT_PACKAGE" where name = '14 堂組合包方案'), 
    (select price from "CREDIT_PACKAGE" where name = '14 堂組合包方案')),

    ((select id from "USER" where email = 'wXlTq@hexschooltest.io'), -- 使用者id, email會設定unique
    (select id from "CREDIT_PACKAGE" where name = '21 堂組合包方案'), -- 組合包id
...
-- 3-3 修改：更新教練的經驗年數，資料需求如下：
    ...
    where user_id in (
      select id from "USER" where email in ('muscle@hexschooltest.io','starplatinum@hexschooltest.io')
    );
...
-- 5-3. 新增：`王小明`再次預約 `李燕容`   的課程，請在`COURSE_BOOKING`新增一筆資料：
    ...
    set cancelled_at = '2024-11-24 17:00:00', status = '課程已取消' -- 設定取消時間及預約狀態
    where user_id = (select id from "USER" where email = 'wXlTq@hexschooltest.io')
    and course_id = (select id from "COURSE" where user_id =(select id from "USER" where email='lee2000@hexschooltest.io'));
...
-- 5-6. 查詢：計算用戶王小明的購買堂數，顯示須包含以下欄位： user_id , total。 (需使用到 SUM 函式與 Group By)
    select 
    "CREDIT_PURCHASE".user_id,  -- 顯示欄位1
    sum("CREDIT_PURCHASE".purchased_credits) as total -- 顯示欄位2 (加總堂數)
      from "CREDIT_PURCHASE" -- 選擇資料表
      where "CREDIT_PURCHASE".user_id = (select id from "USER" where email = 'wXlTq@hexschooltest.io') --篩選出王小明的資料
      group by "CREDIT_PURCHASE".user_id; -- 用user_id分群
...
-- 5-7. 查詢：計算用戶王小明的已使用堂數，顯示須包含以下欄位： user_id , total。 (需使用到 Count 函式與 Group By)
    ...
    and "COURSE_BOOKING".status != '課程已取消' -- 顯示不等於已取消的課
...
-- 注意：多張表合併 會消耗效能 適合少數人查詢使用 不然容易卡住
    ...
    inner join "SKILL" on "SKILL".id = "COACH_LINK_SKILL".skill_id  -- 主鍵 = 外來鍵 
    inner join "COACH" on "COACH".id = "COACH_LINK_SKILL".coach_id  -- 主鍵 = 外來鍵
    inner join "USER" on "USER".id = "COACH".user_id -- 主鍵 = 外來鍵
...
-- 顯示須包含以下欄位： 專長名稱, coach_total
    select 
      "SKILL".name as "專長名稱",
      count(*) as coach_total -- 計算數量
    from "COACH_LINK_SKILL"
    inner join "SKILL" on "SKILL".id = "COACH_LINK_SKILL".skill_id
    group by "SKILL"."name"  -- 分群
    order by coach_total desc  -- 排序
    limit 1; --限制查詢只返回一條結果，即最多教練的技能
...
-- 顯示須包含以下欄位： 總營收
    ...
    from "CREDIT_PURCHASE"
    where purchase_at >= '2024-11-01 00:00:00' and purchase_at <= '2024-11-30 23:59:59';
    
-- 6-5. 查詢：計算 11 月份有預約課程的會員人數（需使用 Distinct，並用 created_at 和 status 欄位統計）
-- 顯示須包含以下欄位： 預約會員人數
    select count("COURSE_BOOKING".user_id) as "預約會員人數"  
    from "COURSE_BOOKING"
    where 
    created_at >= '2024-11-01 00:00:00' and
    created_at <= '2024-11-30 23:59:59' and
    status != '課程已取消';
